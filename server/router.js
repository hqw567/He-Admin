...
/**
 * 规格参数批量删除 batchDelete  idArr Array  id标识
 * sql = "delete from A where id in (1,2,3)"
 */
router.get('/params/batchDelete', (req, res) => {
	let arr = req.query.idArr //[] 数组格式 需要传递数据是 离散的数字格式
	// const sql =`delete from project where id in (?)`;
	let sql = ''
	function fun(arr) {
		//sql =`delete from params where id in (101,102,103)`;
		sql = `delete from params where id in (`
		for (let i = 0; i < arr.length; i++) {
			sql += arr[i] + ',' //101,102,
		}
		sql = sql.slice(0, -1)
